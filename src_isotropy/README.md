...
The reduction of a FAMILY OF VECTORS, which uses all of the above and also BKZ
and slide reduction, lives in `src_latt/VectFamilyReduction.h`: it is a
consumer of everything and so belongs at the top of the dependency stack, the
directory dependency running `src_latt -> src_isotropy`.
