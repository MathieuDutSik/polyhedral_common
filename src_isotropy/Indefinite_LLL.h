...
#ifdef DISABLE_DEBUG_INDEFINITE_LLL
#undef DEBUG_INDEFINITE_LLL
#endif

#ifdef DISABLE_DEBUG_SIMPLE_INDEFINITE_REDUCTION
#undef DEBUG_SIMPLE_INDEFINITE_REDUCTION
#endif
...
ResultIndefiniteLLL<T, Tint> Indefinite_LLL(MyMatrix<T> const &M,
  ...
  while (true) {
    ...
    WriteMatrix(os, B);
#endif
    ResultGramSchmidt_Indefinite<T> ResGS =
        GramSchmidtOrthonormalization(M, B, os);
#ifdef DEBUG_INDEFINITE_LLL
    os << "ILLL: Bstar_norms =";
...
ComputeReductionIndefinite(MyMatrix<T> const &M, bool const &look_for_isotropic,
                           std::ostream &os) {
  int n = M.rows();
  if (n == 0) {
    MyMatrix<Tint> B = IdentityMat<Tint>(0);
...
template <typename T, typename Tint>
ResultReduction<T, Tint> LLLIndefiniteReduction(MyMatrix<T> const &M,
                                                std::ostream &os) {
  int n = M.rows();
  if (n == 1) {
    MyMatrix<Tint> B = IdentityMat<Tint>(n);
...
template <typename T> GraphBitset GetMatrixGraph(MyMatrix<T> const &M) {
  size_t n = M.rows();
  GraphBitset eG(n);
...
MatrixConnectedComponents(MyMatrix<T> const &M) {
  GraphBitset eG = GetMatrixGraph(M);
  std::vector<std::vector<size_t>> LConn = ConnectedComponents_set(eG);
  return LConn;
}

template <typename T> bool IsMatrixConnected(MyMatrix<T> const &M) {
  GraphBitset eG = GetMatrixGraph(M);
  std::vector<std::vector<size_t>> LConn = ConnectedComponents_set(eG);
  return LConn.size() == 1;
}

template <typename T> T get_l1_norm(MyMatrix<T> const &U) {
  int n = U.rows();
  T sum(0);
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
...
SimpleIndefiniteReduction(MyMatrix<T> const &M,
                          [[maybe_unused]] std::ostream &os) {
  int n = M.rows();
  if (n == 1) {
    MyMatrix<Tint> B = IdentityMat<Tint>(n);
...
#ifdef DEBUG_SIMPLE_INDEFINITE_REDUCTION
    os << "ILLL: eval, summary, delta_off=" << delta_off
       << " delta_diag=" << delta_diag << " delta=" << delta << "\n";
#endif
    return delta;
  };
...
#ifdef DEBUG_SIMPLE_INDEFINITE_REDUCTION
              os << "ILLL: x_final=(" << x_final.i << " / " << x_final.j
                 << " / " << x_final.c << ")\n";
#endif
              return x_final;
            }
  ...
  while (true) {
    ...
    if (opt) {
      ...
      os << "ILLL: L1(Mwork)=" << get_l1_norm(Mwork)
         << " L1(pair.second)=" << get_l1_norm(pair.second) << "\n";
      T delta = get_delta(x);
#endif
      update(x);
#ifdef DEBUG_SIMPLE_INDEFINITE_REDUCTION
      ...
      if (norm_prev - delta != norm_next) {
        os << "ILLL: norm_prev=" << norm_prev << " delta=" << delta
           << " norm_next=" << norm_next << "\n";
        os << "ILLL: B=\n";
        WriteMatrix(os, B);
        os << "ILLL: Mwork=\n";
 ...
 */
template <typename T, typename Tint>
ResultReduction<T, Tint> IndefiniteReductionNonDegenerate(MyMatrix<T> const &M,
                                                          std::ostream &os) {
  int n = M.rows();
  MyMatrix<Tint> B = IdentityMat<Tint>(n);
  MyMatrix<T> Mwork = M;
  ...
  auto compute_by_block =
      [&](auto &Min) -> std::optional<ResultReduction<T, Tint>> {
#ifdef DEBUG_INDEFINITE_LLL
    ...
    for (auto &eConn : LConn) {
      ...
      os << "ILLL: IndefiniteReductionNonDegenerate, len=" << len
         << " RankMat(M_conn)=" << rnk << "\n";
#endif
      ResultReduction<T, Tint> res =
          IndefiniteReductionNonDegenerate<T, Tint>(M_conn, os);
      for (int i = 0; i < len; i++) {
        for (int j = 0; j < len; j++) {
          size_t i_big = eConn[i];
          size_t j_big = eConn[j];
  ...
  auto compute_reduction = [&](MyMatrix<T> const &Min,
                               int const &choice) -> ResultReduction<T, Tint> {
    if (choice == 0) {
    ...
    if (choice == 2 || choice == 3) {
#ifdef DEBUG_INDEFINITE_LLL
      os << "ILLL: IndefiniteReductionNonDegenerate, n_plus=" << n_plus
         << " n_minus=" << n_minus << "\n";
#endif
      if (n_plus > 0 && n_minus > 0) {
        MyMatrix<Tint> B = IdentityMat<Tint>(n);
      ...
      auto get_redmat = [&]() -> MyMatrix<Tint> {
        if (choice == 2) {
#ifdef DEBUG_INDEFINITE_LLL
          os << "ILLL: IndefiniteReductionNonDegenerate, before "
                "LLLreduceBasis\n";
#endif
  ...
  while (true) {
...
#ifdef DEBUG_INDEFINITE_LLL
      os << "ILLL: IndefiniteReductionNonDegenerate, norm=" << norm
         << " norm_work=" << norm_work << "\n";
#endif
      if (norm < norm_work) {
        n_operation += 1;
#ifdef DEBUG_INDEFINITE_LLL
        os << "ILLL: choice=" << choice
           << " was successful, |norm old|=" << norm_work
           << " |norm new|=" << norm << "\n";
#endif
        B = res.B * B;
        Mwork = res.Mred;
    ...
    n_iter += 1;
#endif
    // If did nothing then exit. Necessarily we will reach that stage at some
    // point
    if (n_operation < 2) {
      return {B, Mwork};
    }
 ...
 */
template <typename T, typename Tint>
ResultReduction<T, Tint> IndefiniteReduction(MyMatrix<T> const &M,
                                             std::ostream &os) {
  int n = M.rows();
#ifdef DEBUG_INDEFINITE_LLL
  os << "ILLL: IndefiniteReduction, before get_nondegenerate_reduction\n";
#endif
  std::pair<ResultReduction<T, Tint>, int> pair =
      get_nondegenerate_reduction<T, Tint>(M, os);
#ifdef DEBUG_INDEFINITE_LLL
...
#endif
  ResultReduction<T, Tint> res =
      IndefiniteReductionNonDegenerate<T, Tint>(MredA, os);
#ifdef DEBUG_INDEFINITE_LLL
...
template <typename T, typename Tint>
ResultReduction<T, Tint> IndefiniteReduction_opt(MyMatrix<T> const &M,
