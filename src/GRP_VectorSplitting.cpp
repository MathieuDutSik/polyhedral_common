...
int main(int argc, char *argv[])
{
  try {
    if (argc != 3) {
      ...
      std::cerr << "\n";
      std::cerr << "FileIn    : The input file containing n, the modulo, and the list of matrices\n";
    ...
    os << "return [";
    bool IsFirst=true;
    std::function<void(std::vector<int> const&, int const&)> FCT =[&](std::vector<int> const& V, int const& OrbSize) -> void {
      if (IsFirst == false)
	os << ",\n";
      IsFirst=false;
      //
