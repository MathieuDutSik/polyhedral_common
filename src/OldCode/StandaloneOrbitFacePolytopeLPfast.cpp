...
int main(int argc, char *argv[])
{
  ...
  int LevSearch;
  int iret;
  int iLev, nbOrb, nbLev;
  vector<vector<vector<int> > > ListListOrb;
  Initialize_All();
  ...
  ListListOrb=DoMemoryEfficientAllEnum(TheGRP, TheEXT, LevSearch);
  ...
  PrintListListOrb_IntGAP(DATAOUT, ListListOrb);
  iret=fclose(DATAOUT);
  nbLev=ListListOrb.size();
  fprintf(stderr, "nbLev=%d\n", nbLev);
  for (iLev=0; iLev<nbLev; iLev++)
    {
      nbOrb=ListListOrb[iLev].size();
      fprintf(stderr, "  iLev=%d nbOrb=%d\n", iLev, nbOrb);
    }
  Free_All();
}
