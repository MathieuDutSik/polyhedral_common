...
template<typename T>
EquivTest<MyMatrix<int>> PERF_TestEquivalence(LinSpaceMatrix<T> const&LinSpa,
				       MyMatrix<T> const&ePerf1, MyMatrix<T> const&ePerf2,
				       MyMatrix<int> const& SHV1, MyMatrix<int> const& SHV2)
{
  ...
  if (IsIntegralMatrix(M3)) {
    MyMatrix<int> eMatEquiv=TMat_ConvertToInt(M3);
...
template<typename T>
std::istream& operator>>(std::istream& is, SimplePerfect<T>& obj)
{
...
template<typename T>
std::ostream& operator<<(std::ostream& os, SimplePerfect<T> const& obj)
{
...
template <typename T>
struct invariant_info<SimplePerfect<T>> {
  typedef SimplePerfectInv<T> invariant_type;
};

template <typename T>
struct equiv_info<SimplePerfect<T>> {
...
template<typename T>
EquivTest<MyMatrix<int>> SimplePerfect_TestEquivalence(
		      DataLinSpa<T> const&eData,
		      MyMatrix<T> const& Gram1,
		      MyMatrix<T> const& Gram2)
{
  Tshortest<T,int> RecSHV1=T_ShortestVector<T,int>(Gram1);
  Tshortest<T,int> RecSHV2=T_ShortestVector<T,int>(Gram2);
  MyMatrix<T> T_SHV1=TMat_ConvertFromInt<T>(RecSHV1.SHV);
  MyMatrix<T> T_SHV2=TMat_ConvertFromInt<T>(RecSHV2.SHV);
...
template<typename T>
TheGroupFormat SimplePerfect_Stabilizer(DataLinSpa<T> const& eData, MyMatrix<T> const& Gram, Tshortest<T,int> const& RecSHV)
{
  // 
  ...
  MyMatrix<T> ListMatVectB=MatrixFromVectorFamily(ListMatVect);
  MyMatrix<T> T_SHV=TMat_ConvertFromInt<T>(RecSHV.SHV);
  std::function<bool(MyMatrix<T>)> IsMatrixCorrect=[&](MyMatrix<T> const& M) -> bool {
    if (!IsIntegralMatrix(M))
      return false;
    if (eData.NeedCheckStabilization) {
...
template<typename T, typename Tint>
std::vector<SimplePerfect<T>> EnumerationPerfectMatrices(
	   ...
	   PolyHeuristic<mpz_class> const& AllArr)
{
  ...
  std::function<void(TrivialBalinski &,SimplePerfect<T> const&,SimplePerfectInv<T> const&,std::ostream&)> UpgradeBalinskiStat=[](TrivialBalinski const& eStat, SimplePerfect<T> const& eEnt, SimplePerfectInv<T> const& eInv, std::ostream&os) -> void {
  };
  std::function<EquivTest<MyMatrix<int>>(SimplePerfect<T> const&,SimplePerfect<T> const&)> fEquiv=[&](SimplePerfect<T> const& x, SimplePerfect<T> const& y) -> EquivTest<MyMatrix<int>> {
    return SimplePerfect_TestEquivalence<T>(eData, x.Gram, y.Gram);
  };
  NewEnumerationWork<SimplePerfect<T>> ListOrbit(AllArr.Saving, AllArr.eMemory, eData.PrefixPerfect, CompFCT, UpgradeBalinskiStat, fEquiv, MProc.GetO(TheId));
  auto FuncInsert=[&](SimplePerfect<T> const& x, std::ostream&os) -> int {
  ...
  auto CompSimplePerfectAdjacency=[&](int const& MyId, std::ostream& os) -> void {
	...
	break;
      if (IsComplete)
	break;
      SimplePerfect<T> ePERF=ListOrbit.GetRepresentative(eEntry);
  ...
  std::vector<SimplePerfect<T>> ReturnData;
  if (eData.ReturnAll) {
...
void TreatPerfectLatticesEntry(FullNamelist const& eFull)
{
  ...
  //
  std::vector<SimplePerfect<T>> LPerf=EnumerationPerfectMatrices<T>(MProc, TheId,
