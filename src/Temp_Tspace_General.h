...
template<typename T>
void T_GetGramMatrixAutomorphismGroup(MyMatrix<T> const& eMat, T const& TheTol, TheGroupFormat &GRPperm, std::vector<MyMatrix<int> > &ListMatrGens)
{
  T MaxDet=T_GRAM_GetUpperBound(eMat);
  MyMatrix<int> ListShort=T_ShortVector(eMat, MaxDet);
  MyMatrix<T> ListShort_T=TMat_ConvertFromInt<T>(ListShort);
  ...
  for (auto & eGen : GRPperm.group->S) {
    ...
    MyMatrix<int> M3_I=TMat_ConvertToInt(M3_T);
