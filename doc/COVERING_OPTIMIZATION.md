...
was already the right method for the *descent* -- joint, smooth,
quasi-Newton -- and the effort belongs on the tessellation, not on replacing
it.

**The LP direction step and the rigidity certificate.** What the smooth
soft-max descent cannot do is decide, at a limit point, whether the true
non-smooth objective can still be lowered: it optimizes a surrogate and
sometimes stops slightly short on the non-smooth ridge `mu^2 = max_i r_i^2`.
The exact question is a linear program. Linearize each active circumradius,
`r_i^2(x_0 + d) ~ r_i^2(x_0) + g_i . d` with `g_i = grad r_i^2` in the joint
coordinates `x = (Q sym-basis entries, coset entries)`, and solve

```
minimize   (n/2)/mu^2 * s  -  (1/2) gdet . d
over       d in R^dim,  s in R
subject to g_i . d <= s   for every active orbit i,   -rho <= d_k <= rho
```

where `gdet = grad log det Q` and `s` plays the role of `d(mu^2)`. The point
`d = 0, s = 0` is always feasible with objective 0, so the optimum is `<= 0`:
a strictly negative value is a guaranteed joint descent direction of
`log Theta`, and an optimum of 0 certifies that **no** first-order descent
direction exists -- the configuration is *rigid* (jammed), a first-order
stationary point of the true covering density where `0` lies in the cone
generated by the active circumradius gradients (the covering analogue of a
perfect form). The objective is homogeneous in the box radius `rho`, so
`-(optimum)/rho` is a scale-free descent rate; `certify` reports it, and it
separates cleanly -- genuinely rigid configurations score `~1e-14` while
shallow non-smooth stalls score `1e-4` to `1e0`, ten or more orders apart. A
small `rho` is used so the linearization stays valid: a large box lets the
linear model "see" descent that would in fact activate other radii once the
step is taken. The LP is solved by a small dense primal simplex (`SimplexMin`,
all-slack start since every right-hand side is non-negative) with no external
dependency.

One numerical subtlety is essential and was, for a while, the whole reason the
search "did not converge". This LP is massively degenerate -- every active row
has right-hand side 0 -- and a dense tableau simplex accumulates enough roundoff
that the basic point it returns can be infeasible by a hair, its internal `s`
sitting just below `max_i g_i . d`. The reported objective is then spuriously
negative, on the *same order* as a genuine shallow descent (`~1e-5`), so a
configuration that is in fact rigid looks like it still has a `1e-5` descent
direction that the line search can never realize -- every seed appears to
"stall". The cure is to never trust the simplex's reported objective: recompute
the direction's true first-order effect on `log Theta` from the returned `d`
using the real active radii, and clamp a roundoff-induced ascent to 0. With
that, the rate a rigid configuration reports drops from a phantom `1e-5` to `0`,
an independent random-direction probe agrees the point is a local minimum, and a
`multistart-lp` sweep that had certified 0 of 10 seeds rigid certifies 9 of 10
-- the configurations were rigid all along and the roundoff was hiding it.

As an *optimizer* the LP direction step (`JointDescentLP`, driver mode
`descend-lp`) works -- it drives `log Theta` down monotonically and, unlike
the alternation, it moves `Q` and `c` jointly so it never jams -- but it is
first-order and therefore converges more slowly per iteration than L-BFGS near
a minimum, the same reason the earlier steepest-descent variants lost. The
productive division of labour is therefore `polish`: **L-BFGS descends fast to
the neighbourhood of a minimum, then the LP step finishes along the non-smooth
ridge and certifies rigidity.** The `multistart-lp` sweep runs this per random
seed and writes out every configuration it can certify rigid; because rigidity
is decided only by the linearized first-order test (never by a mere line-search
stall), the rigid configurations it reports are genuine -- the `2.16006`
continuum optimum comes out rigid with `rate = 0` and 192 of its 288 orbits
active, and the earlier `2.4133` multistart winner comes out rigid with 170
active, whereas a seed whose descent genuinely has not reached a stationary
point is correctly reported non-rigid (a positive recomputed rate) and is not
written out. Most soft-max L-BFGS limits turn out to be rigid already: the
soft-max surrogate's smoothing biases its minimizer off the true stationary
point by an amount that is invisible here, because the certificate is the LP on
the *true* objective, not the surrogate -- what looked like a persistent
`1e-5` residual was the simplex roundoff above, not a real displacement. These rigid
configurations are the objects of interest: being isolated solutions of the
polynomial KKT system (active radii equal, `gdet` a non-negative combination
of the active gradients) they are expected to have algebraic coordinates, as
the `2.16006 in Q(sqrt 2)` optimum does.
