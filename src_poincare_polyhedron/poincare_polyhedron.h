...
template <typename T>
DataEXT<T> DirectDataExtComputation(MyMatrix<T> const &FAC,
...
public:
  ...
  bool InsertStabilizerGenerator(CombElt<T> const &eElt, std::ostream &os) {
    if (IsPresentInStabilizer(eElt))
      return false;
    os << "InsertStabilizerGenerator 1 |stabilizerElt|=" << stabilizerElt.size()
       << "\n";
    std::vector<CombElt<T>> ExtListGen = stabilizerElt;
    ExtListGen.push_back(eElt);
    os << "InsertStabilizerGenerator 2 |ExtListGen|=" << ExtListGen.size()
       << "\n";
    stabilizerElt = GroupGeneration(ExtListGen);
    os << "InsertStabilizerGenerator 3 |stabilizerElt|=" << stabilizerElt.size()
       << "\n";
  ...
  MyMatrix<T> GetFAC(std::ostream &os) const {
    ...
    for (int i_mat = 0; i_mat < n_mat; i_mat++) {
      ...
      if (val != 0) {
        size_t j_mat = val - 1;
        os << "Collision found between i_mat=" << i_mat << " j_mat=" << j_mat
           << "\n";
...
GetMissingInverseElement(StepEnum<T> const &se, DataFAC<T> const &datafac,
...
// missed element.
template <typename T>
AdjacencyInfo<T> GetMissingFacetMatchingElement_LP(
    StepEnum<T> const &se, DataFAC<T> const &datafac,
    ShortVectorGroup<T> const &svg, std::ostream &os) {
  HumanTime time;
  //
  ...
  for (int i_fac = 0; i_fac < n_fac; i_fac++) {
    ...
    for (int i_adj_img = 0; i_adj_img < n_adj_img; i_adj_img++) {
      ...
      if (!SolCompl.SolNonnegative) {
        MyVector<T> eEXTred =
            unfold_opt(SolCompl.ExtremeRay, "Failed to find an extreme ray");
...
template <typename T>
AdjacencyInfo<T> GetMissingFacetMatchingElement_DD(
    StepEnum<T> const &se, DataFAC<T> const &datafac,
    std::string const &eCommand_DD, ShortVectorGroup<T> const &svg,
    std::ostream &os) {
...
template <typename T>
AdjacencyInfo<T> GetMissingFacetMatchingElement(
    StepEnum<T> const &se, DataFAC<T> const &datafac,
    std::string const &method_adjacent, std::string const &eCommand_DD,
    ShortVectorGroup<T> const &svg, std::ostream &os) {
...
template <typename T>
AdjacencyInfo<T> ComputeAdjacencyInfo(StepEnum<T> const &se,
  ...
  return GetMissingFacetMatchingElement(se, datafac, method_adjacent,
                                        eCommand_DD, svg, os);
}
...
GetMissing_TypeI_Gen2(StepEnum<T> const &se, DataFAC<T> const &datafac,
                      CombElt<T> const &TestElt, int const &max_iter,
                      std::ostream &os) {
  struct ResultOptim {
    T the_scal;
    MyVector<T> the_x;
  ...
  while (true) {
    ...
    if (res.result == 1) {
      ...
      if (test_stab) {
        ...
        os << "It stabilizes but is not already known so interesting by "
              "itself\n";
        return ResidualElt;
      }
    }
...
template <typename T>
std::optional<CombElt<T>>
GetMissing_TypeI_Gen1(StepEnum<T> const &se, DataFAC<T> const &datafac,
                      CombElt<T> const &TestElt, int const &max_iter,
                      std::ostream &os) {
    ...
    if (strategy == "strategy2") {
      ...
      for (auto &eAdj : datafac.ListAdj) {
        ...
        if (test_scal < curr_scal) {
          ...
          os << "    Improving scalar product from curr_scal_d=" << curr_scal_d
             << " to test_scal_d=" << test_scal_d << "\n";
          curr_x = test_x;
          curr_scal = test_scal;
...
template <typename T>
std::optional<CombElt<T>>
GetMissing_TypeI(StepEnum<T> const &se, DataFAC<T> const &datafac,
                 CombElt<T> const &TestElt, int const &max_iter,
...
template <typename T>
std::vector<CombElt<T>>
ReorderElements(std::vector<CombElt<T>> const &l_elt_pre, std::ostream &os) {
...
void InsertAndCheckRedundancy(StepEnum<T> &se,
                              ...
                              RecOption const &rec_option, std::ostream &os) {
...
GetGroupPresentation(StepEnum<T> const &se, AdjacencyInfo<T> const &ai,
                     std::ostream &os) {
  int n = se.x.size();
...
StepEnum<T> compute_step_enum(RecOption const &rec_option, std::ostream &os) {
  ...
  auto f_init = [&]() -> StepEnum<T> {
    ...
    if (rec_option.Approach == "Restart") {
      ...
      std::optional<StepEnum<T>> opt =
          SingleData_LoadLast<StepEnum<T>>(PrefixStepenum);
      if (opt) {
        return *opt;
      } else {
