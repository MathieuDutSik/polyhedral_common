...
template<typename T>
using PairElt<T> = std::pair<TrackGroup,MyMatrix<T>>;
...
template<typename T>
AdjacencyInfo<T> ComputeAdjacencyInfo(MyVector<T> const& x, StepEnum<T> const& se, std::string const& eCommand) {
  int n = x.size();
...
template<typename T>
AdjacencyInfo<T> IterativePoincareRefinement(DataPoincare<T> const& dp, RecOption const& ro) {
  MyVector<T> x = dp.x;
