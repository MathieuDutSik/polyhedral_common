...
template <typename T>
void write_deformation_gap(std::ostream &os_out,
  ...
  // Only exact rationals go into the GAP record. The normalized G(t) and its
  // derivatives (irrational, floating point) are printed to stderr; they are
  // recoverable from these fields via G(t) = (1/n) det(t)^(-1/n) SecMoment(t).
...
void process(std::string const &Qfile, std::string const &Hfile,
  ...
  if (OutFile != "" && OutFile != "stderr") {
    ...
    write_deformation_gap(os_out, der);
  }
}
...
void process_B(std::string const &Qfile, std::string const &Hfile,
  ...
  return process<T, Tint, Tgroup>(Qfile, Hfile, OutFile, std::cerr);
}

void process_C(std::string const &arith, std::string const &Qfile,
               std::string const &Hfile, std::string const &OutFile) {
  if (arith == "gmp") {
    using T = mpq_class;
    using Tint = mpz_class;
    return process_B<T, Tint>(Qfile, Hfile, OutFile);
  }
  if (arith == "safe") {
...
int main(int argc, char *argv[]) {
  ...
  try {
    if (argc != 4 && argc != 5) {
      ...
      std::cerr << "     or\n";
      std::cerr
          << "LATT_QuantizationDeformation [arith] [Qfile] [Hfile] [OutFile]\n";
      ...
      std::cerr << "     Hfile    the symmetric perturbation H (any symmetric "
                   "form)\n";
      std::cerr << "     OutFile  optional GAP-readable result file\n";
      return -1;
    }
    ...
    process_C(arith, Qfile, Hfile, OutFile);
