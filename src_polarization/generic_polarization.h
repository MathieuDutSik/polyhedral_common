...
// Maybe use MaxNorm?
dual2nd Compute_Polarization(int const &n, std::vector<int> const &Ranges,
                             std::vector<dual2nd> const &cA,
...
dual2nd Senergy(int const &n, std::vector<int> const &Ranges,
                std::vector<dual2nd> const &A,
                std::function<dual2nd(dual2nd const &)> const &f) {
  std::vector<int> Ranges_Ext(n);
  for (int i = 0; i < n; i++)
    Ranges_Ext[i] = 1 + 2 * Ranges[i];
...
*/
dual2nd determinantSymMat_function(int const &n,
                                   std::vector<dual2nd> const &Acoeff) {
...
/*
MyMatrix<double> NewtonIterationOptimization(MyMatrix<double> const& Ainit,
std::function<dual2nd(dual2nd const&)> const& f)
{
  int n=Ainit.rows();
