...
template<typename T, typename Tint>
PosRelRes<T> SearchForExistenceStrictPositiveRelation(MyMatrix<Tint> const& SHV, MyMatrix<T> const& eMat)
{
...
TestStrictPositivity<T,Tint> TestingAttemptStrictPositivity(MyMatrix<T> const& eMat)
{
  ...
  while(true) {
    ...
    MyMatrix<T> ConeClassical = GetNakedPerfectConeClassical<T,Tint>(eNaked.SHVred);
    std::cerr << "Before SearchPositiveRelationSimple RankMat(ConeClassical)=" << RankMat(ConeClassical) << " nbIter=" << nbIter << "\n";
    PosRelRes<T> eRel=SearchForExistenceStrictPositiveRelation(eNaked.SHVred, eMat);
	...
	for (int u=0; u<n; u++)
	  for (int v=0; v<n; v++)
	    TotalSum(u, v) += eVal * RealizingFamily(iIdx, u) * RealizingFamily(iIdx, v);
      }
    ...
    std::cerr << "eFace.count=" << eFace.count() << " eFace.size=" << eFace.size() << "\n";
...
template<typename T, typename Tint>
void WriteStrictPositivityResult(std::ostream & os, TestStrictPositivity<T,Tint> const& StrictPos)
{
  ...
  if (StrictPos.result) {
    ...
    os << "Following matrix has negative scalar product with eMat and is copositive:\n";
