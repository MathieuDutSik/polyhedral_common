...
void ComputeSphericalSolutions(const MyMatrix<T> &GramMat,
  ...
  auto f_insert = [&](const MyVector<Tint> &V_y, const T &min) -> bool {
    ...
    if (min == norm) {
      ...
      //      std::cerr << "ComputeSphericalSolutions norm_red=" << norm_red <<
      //      " norm_tot=" << norm_tot << " min=" << min << "\n";
...
private:
  ...
  Tint get_k() const {
    ...
    for (auto &k : Vtot.root_lengths) {
      ...
      //      std::cerr << "IterateRootDecompositions k=" << k << " val=" << val
      //      << " k_d=" << k_d << " candidates.at(k)=" << candidates.at(k) << "
      //      V2=" << V2 << "\n";
      if (!we_found) {
        we_found = true;
        minval_d = val_d;
...
FindRoot_filter(const VinbergTot<T, Tint> &Vtot, const MyVector<Tint> &a,
  ...
  auto fct_CVP = [&]() -> void {
    ...
    auto f_ins_root = [&](const MyVector<Tint> &V) -> void {
      ...
      //      std::cerr << "Inserting a vector V=" << V << " norm=" << norm << "
      //      k=" << k << "\n";
      if (norm != k) {
