...
template<typename T>
T Linf_Norm(MyVector<T> const& b)
{
  T eNorm=0;
  int siz=b.size();
  for (int i=0;i<siz; i++) {
...
RecSparse<T> AMP_linear_operators(MySparseMatrix<T> const& preA0)
{
  ...
  std::function<MyVector<T>(MyVector<T> const&)> f1=[=](MyVector<T> const& x) -> MyVector<T> {
  ...
  std::function<MyVector<T>(MyVector<T> const&)> f2=[=](MyVector<T> const& x) -> MyVector<T> {
    //    std::cerr << "f2: n=" << n << " m=" << m << "\n";
    //    std::cerr << "(rows/cols)x = " << x.rows() << " / " << x.cols() << "\n";
    //    std::cerr << "(rows/cols)A0 = " << A0.rows() << " / " << A0.cols() << "\n";
...
template<typename T>
OutSolver<T> AMP_yall1(RecSparse<T> const& eRecSparse, MyVector<T> const& b, RecOptSparse<T> const& eRecOpt)
{
  /*
...
template<typename T>
bool check_orth(RecSparse<T> const& eRecSparse)
{
  int n=eRecSparse.n;
  MyVector<T> s1(n);
  for (int i=0; i<n; i++) {
...
template<typename T>
MyVector<T> proj2box(MyVector<T> const& z,MyVector<T> const& w,bool const& nonneg,T const& nu, int const& m)
{
  size_t siz=z.size();
...
template<typename T>
T T_sumAbs(MyVector<T> const& a, MyVector<T> const& b)
{
  T eSum;
  eSum=0;
  size_t siz=a.size();
...
template<typename T>
OutSolver<T> yall1_solve(RecSparse<T> const& eRecSparse, MyVector<T> const& b, MyVector<T> const& x0, MyVector<T> const& z0, RecOptSparse<T> const& eRecOpt)
{
  /*
...
template<typename T>
MyVector<T> AMP_SolutionSparseSystem(MySparseMatrix<T> const& SpMat, MyVector<T> const& eVect)
{
  ...
  eRecOpt.mu=0;
  eRecOpt.nu=0;
  eRecOpt.eps=2.2204e-16; // The value of eps in matlab which is a default value (CRAZY Matlab!)
  eRecOpt.rho=0;
  eRecOpt.tol=1e-10;
  eRecOpt.gamma=0;
