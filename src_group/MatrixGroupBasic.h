...
#ifdef DISABLE_DEBUG_MATRIX_GROUP_BASIC
#undef DEBUG_MATRIX_GROUP_BASIC
#endif
...
MatrixIntegral_GetInvariantSpace(int const &n,
  ...
  while (true) {
    ...
    if (NewDet == TheDet) {
      ...
      os << "MATGRPBAS: MatrixIntegral_GetInvariantSpace, returning after "
            "n_iter="
         << iter << " TheDet=" << TheDet << "\n";
#endif
#ifdef TIMINGS_MATRIX_GROUP_BASIC
    ...
    TheDet = NewDet;
#ifdef DEBUG_MATRIX_GROUP_BASIC
    os << "MATGRPBAS: MatrixIntegral_GetInvariantSpace, iter=" << iter
       << " TheDet=" << TheDet << "\n";
    iter += 1;
#endif
  }
...
&a, const Fprod &f_prod, std::ostream &os) { std::vector<T2> orb =
OrbitComputation(ListGen, a, f_prod, os); std::sort(orb.begin(), orb.end());
  return orb;
}
*/
