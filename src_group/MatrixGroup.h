...
/*
  The matrix realizing a permutation of the faithful family.

  The realization needs a selection of independent rows of the family and
  the inverse of the selected submatrix. Both depend only on the family,
  which is fixed here, so they are computed once in the solver rather than
  on every call as the plain FindTransformationRing does: with the vector
  families of the lattice code, thousands of rows against a dimension of a
  few dozen, that preparation dominates the n x n solve, and a stabilizer
  computation calls this once per generator.

  solve_notcheck is the right entry point: the permutations reaching this
  point are elements of a group generated by permutations that come from
  actual matrices acting on the family, so they are realized by
  construction. SANITY_CHECK_PERM_FCT still verifies it. An empty optional
  therefore means only that the realizing matrix is not representable over
  the ring T.

  EXTfaithful is held by reference, as in FindTransformationSolver: the
  object is local to one computation and must not outlive the helper that
  produced it. This also spares the copy of the whole family that every
  construction used to pay.
 */
template <typename T, typename Telt> struct PreImager_Finite {
  MyMatrix<T> const &EXTfaithful;
  FindTransformationSolver<T> solver;
  PreImager_Finite(MyMatrix<T> const &_EXTfaithful)
      : EXTfaithful(_EXTfaithful), solver(_EXTfaithful) {}
  MyMatrix<T> pre_image_elt(Telt const &elt,
                            [[maybe_unused]] std::ostream &os) const {
    using Tidx = typename Telt::Tidx;
    // elt may act on a larger domain than the family, the rows of
    // EXTfaithful forming the first block, which it preserves.
    auto f = [&](int iRow) -> int {
      Tidx pos = static_cast<Tidx>(iRow);
      return static_cast<int>(OnPoints(pos, elt));
    };
    std::optional<MyMatrix<T>> opt = solver.solve_notcheck_f(EXTfaithful, f);
    if (!opt) {
