...
void process(std::string const &FileMatrGroup, std::string const &OutFormat,
  ...
  for (size_t i_gen = 0; i_gen < n_gen; i_gen++) {
    ...
    std::pair<Ttype,Ttype> pair_direct_inv{pair, pair_inv};
    TcombPair<Ttype, Tnorm> comb_pair{pair_direct_inv, f_complexity(pair)};
    
    bool success = online_kernel.insert_generator(comb_pair);
    if (!success) {
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    std::cerr << "Normal termination of GRP_MatrixGroupPermSimplificationOnline\n";
  } catch (TerminalException const &e) {
    std::cerr << "Error in GRP_MatrixGroupPermSimplificationOnline\n";
    exit(e.eVal);
  }
  runtime(time);
}