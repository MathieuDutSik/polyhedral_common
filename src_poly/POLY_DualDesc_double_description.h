...
/*
  ...
  following
  --- T. S. Motzkin, H. Raiffa, G. L. Thompson, R. M. Thrall, "The double
      description method", Contributions to the theory of games II,
      Annals of Mathematics Studies 28 (1953), 51--73,
  --- K. Fukuda, A. Prodon, "Double Description Method Revisited",
      Combinatorics and Computer Science, LNCS 1120 (1996), 91--111,
      for the combinatorial adjacency test and the insertion order.
  ...
  returns for each facet the set of rows lying on it; DualDescription
  returns the facet vectors.

  The insertion order of the rows is the lexicographic-minimum order,
  the robust default of Fukuda-Prodon.
 */
...
std::vector<RayEntry<T>> DoubleDescription_Kernel(MyMatrix<T> const &EXT,
  ...
  for (auto &iRow : order) {
    ...
    int n_neg = 0;
    for (auto &ray : rays) {
      // The accumulation goes directly into ray.val through the fused
      // AddMul, and the capacity of val is reused across the rows.
      ray.val = 0;
      for (int j = 0; j < d; j++) {
        AddMul(ray.val, EXT(iRow, j), ray.coord(j));
      }
      if (ray.val < 0) {
        n_neg++;
      } else {
...
/*
  The combinatorial adjacency graph of a family of incidence sets over a
  common ground set: two members are adjacent when no third member
  contains the intersection of their sets. The test is the one of the
  double description kernel, on packed words with the scan restricted to
  the members containing the least populated element of the intersection.
  count_min is a necessary size of the intersection, used as a prefilter.
  The status entries refine the graph: 0 is a normal member, 1 is a
  redundant member (no edges at all, does not block adjacencies), 2 is a
  dominant member (adjacent to every non-redundant member, does not
  block adjacencies).
*/
inline GraphBitset
CombinatorialAdjacencyGraph(std::vector<Face> const &sets, size_t const &ground,
                            size_t const &count_min,
                            std::vector<uint8_t> const &status) {
  size_t n_set = sets.size();
  GraphBitset GR(n_set);
  auto set_edge = [&](size_t i, size_t j) -> void {
    GR.AddAdjacent(i, j);
    GR.AddAdjacent(j, i);
  };
  size_t n_words = (ground + 63) / 64;
  std::vector<uint64_t> zw(n_set * n_words, 0);
  std::vector<std::vector<int>> elt_tight(ground);
  for (size_t i = 0; i < n_set; i++) {
    boost::dynamic_bitset<>::size_type pos = sets[i].find_first();
    while (pos != boost::dynamic_bitset<>::npos) {
      zw[i * n_words + pos / 64] |= uint64_t(1) << (pos % 64);
      if (status[i] == 0) {
        elt_tight[pos].push_back(i);
      }
      pos = sets[i].find_next(pos);
    }
  }
  std::vector<uint64_t> common(n_words);
  for (size_t i1 = 0; i1 < n_set; i1++) {
    if (status[i1] == 1) {
      continue;
    }
    uint64_t const *w1 = zw.data() + i1 * n_words;
    for (size_t i2 = i1 + 1; i2 < n_set; i2++) {
      if (status[i2] == 1) {
        continue;
      }
      if (status[i1] == 2 || status[i2] == 2) {
        set_edge(i1, i2);
        continue;
      }
      uint64_t const *w2 = zw.data() + i2 * n_words;
      ...
      for (size_t k = 0; k < n_words; k++) {
        ...
        while (w != 0) {
          int pos = 64 * k + __builtin_ctzll(w);
          w &= w - 1;
          if (elt_tight[pos].size() < best_size) {
            best_size = elt_tight[pos].size();
            best_elt = pos;
          }
        }
      }
      bool adjacent = true;
      if (best_elt >= 0) {
        for (auto &i3 : elt_tight[best_elt]) {
          size_t i3_st = static_cast<size_t>(i3);
          if (i3_st == i1 || i3_st == i2) {
            continue;
          }
          uint64_t const *w3 = zw.data() + i3_st * n_words;
          bool subset = true;
          for (size_t k = 0; k < n_words; k++) {
            if ((common[k] & ~w3[k]) != 0) {
              subset = false;
...
// The dual description together with the adjacency graphs. EXT is the
// list of facets. RidgeGraph is the adjacency between the facets (two
// facets are adjacent when they share a ridge). SkelGraph is the
// adjacency between the input rows: redundant rows (incidence contained
// in the incidence of another row, with the smallest index of a group of
// duplicates surviving) are isolated vertices, and rows incident to
// every facet are adjacent to every non-redundant row.
template <typename T> struct DDA {
  MyMatrix<T> EXT;
  std::optional<GraphBitset> SkelGraph;
  std::optional<GraphBitset> RidgeGraph;
};

template <typename T>
DDA<T> DualDescriptionAdjacencies(MyMatrix<T> const &TheEXT, bool get_ridge,
                                  bool get_skel, std::ostream &os) {
#ifdef TIMINGS_DOUBLE_DESCRIPTION
  MicrosecondTime time;
#endif
  int nbRow = TheEXT.rows();
  std::pair<MyMatrix<T>, vectface> pair = DualDescription_pair(TheEXT, os);
  size_t n_facet = pair.second.size();
  // The intersection size prefilters: two adjacent facets span a face of
  // the primal cone of codimension 2, generated by at least rank(EXT)-2
  // of the input rows; two adjacent input rays span a face of the dual
  // cone of codimension 2 in it, containing at least rank(FAC)-2 of the
  // facets. The two ranks differ when the primal cone has a lineality
  // space.
  size_t rank_ext = RankMat(TheEXT);
  size_t count_min_ridge = (rank_ext >= 2) ? rank_ext - 2 : 0;
  size_t rank_fac = (n_facet > 0) ? RankMat(pair.first) : 0;
  size_t count_min_skel = (rank_fac >= 2) ? rank_fac - 2 : 0;
  std::optional<GraphBitset> RidgeGraph;
  if (get_ridge) {
    // The adjacency of the facets from their incidence sets: all the
    // facets are extreme, so no special status occurs.
    std::vector<Face> sets;
    sets.reserve(n_facet);
    for (auto eFace : pair.second) {
      sets.push_back(eFace);
    }
    std::vector<uint8_t> status(n_facet, 0);
    RidgeGraph =
        CombinatorialAdjacencyGraph(sets, nbRow, count_min_ridge, status);
#ifdef TIMINGS_DOUBLE_DESCRIPTION
    os << "|DOUBLEDESC: RidgeGraph|=" << time << "\n";
#endif
  }
  std::optional<GraphBitset> SkelGraph;
  if (get_skel) {
    // The transposed incidence: for each input row the set of facets it
    // lies on.
    std::vector<Face> Ainc(nbRow, Face(n_facet));
    size_t i_facet = 0;
    for (auto eFace : pair.second) {
    ...
    // The rows with empty incidence are redundant, with precedence over
    // the dominant classification (both apply when there is no facet at
    // all). The dominant rows lie on every facet. The other redundant
    // rows have their incidence contained in the incidence of another
    // surviving row; the decreasing order keeps the smallest index of a
    // group of duplicates.
    std::vector<uint8_t> status(nbRow, 0);
    for (int iRow = 0; iRow < nbRow; iRow++) {
      if (Ainc[iRow].count() == 0) {
        status[iRow] = 1;
      } else {
        if (Ainc[iRow].count() == n_facet) {
          status[iRow] = 2;
        }
      }
    }
    for (int iRow = nbRow - 1; iRow >= 0; iRow--) {
      if (status[iRow] != 0) {
        continue;
      }
      for (int kRow = 0; kRow < nbRow; kRow++) {
        if (kRow != iRow && status[kRow] == 0 &&
            Ainc[iRow].is_subset_of(Ainc[kRow])) {
          status[iRow] = 1;
          break;
        }
      }
    ...
    SkelGraph =
        CombinatorialAdjacencyGraph(Ainc, n_facet, count_min_skel, status);
#ifdef TIMINGS_DOUBLE_DESCRIPTION
    os << "|DOUBLEDESC: SkelGraph|=" << time << "\n";
#endif
  }
  return {std::move(pair.first), std::move(SkelGraph), std::move(RidgeGraph)};
}

// Process each facet as a pair (incidence, facet vector) through the
// f_process callback. Matches the conventions of
// lrs::DualDescriptionFaceIneq.
template <typename T, typename Fprocess>
void DualDescriptionFaceIneq(MyMatrix<T> const &EXT, Fprocess f_process,
                             std::ostream &os) {
  std::pair<MyMatrix<T>, vectface> pair = DualDescription_pair(EXT, os);
  int n_facet = pair.first.rows();
  for (int i_facet = 0; i_facet < n_facet; i_facet++) {
    MyVector<T> V = GetMatrixRow(pair.first, i_facet);
    std::pair<Face, MyVector<T>> pair_face{pair.second[i_facet],
                                           std::move(V)};
    f_process(pair_face);
  }
}
