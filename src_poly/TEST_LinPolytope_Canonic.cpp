...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    //
    MyMatrix<Tint> EXT_can = LinPolytope_CanonicForm<Tint, use_scheme>(EXT, std::cerr);
    std::cerr
        << "------------------------------------------------------------\n";
    //
    ...
    for (int i_iter = 0; i_iter < n_iter; i_iter++) {
      ...
      MyMatrix<Tint> EXT2_can = LinPolytope_CanonicForm<Tint, use_scheme>(EXT2, std::cerr);
      std::cerr
          << "------------------------------------------------------------\n";
