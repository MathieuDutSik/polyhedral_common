...
#ifdef DISABLE_DEBUG_LINEAR_PROGRAM
#undef DEBUG_LINEAR_PROGRAM
#endif

#ifdef DISABLE_DEBUG_POLYTOPIZATION
#undef DEBUG_POLYTOPIZATION
#endif

#ifdef DISABLE_DEBUG_SEARCH_POSITIVE_RELATION
#undef DEBUG_SEARCH_POSITIVE_RELATION
#endif

#ifdef DISABLE_DEBUG_GEOMETRICALLY_UNIQUE
#undef DEBUG_GEOMETRICALLY_UNIQUE
#endif

#ifdef DISABLE_DEBUG_FULL_RANK_FACET_SET
#undef DEBUG_FULL_RANK_FACET_SET
#endif

#ifdef DISABLE_DEBUG_FIND_SINGLE_VERTEX
#undef DEBUG_FIND_SINGLE_VERTEX
#endif
...
MyMatrix<T> Polytopization(MyMatrix<T> const &EXT, std::ostream &os) {
  ...
  if (!eSol.DirectSolution) {
    ...
    std::cerr
        << "LP: A full dimensional polytope, i.e. that the inequalitity\n";
...
#ifdef DEBUG_FIND_SINGLE_VERTEX
        os << "LP: Kernel_FindSingleVertex, returning Inc_contrib at n_iter="
           << n_iter << "\n";
#endif
        return Inc_contrib;
      }
...
#ifdef DEBUG_FIND_SINGLE_VERTEX
      os << "LP: Kernel_FindSingleVertex, returning Inc_eSum at n_iter="
         << n_iter << "\n";
#endif
      return Inc_eSum;
    }
  ...
  if (pos.has_internal_vector) {
    if (!pos.eTestExist) {
      if (!pos.InternalVector.has_value()) {
        std::cerr
            << "LP: There is an internal vector while eTestExist is true\n";
        throw TerminalException{1};
      }
      ...
      if (pos.InternalVector.has_value()) {
        std::cerr
            << "LP: There is no internal vector while eTestExist is false\n";
        throw TerminalException{1};
      }
    }
  }
...
template <typename T>
std::optional<MyVector<T>>
SolutionMatStrictlyPositive(MyMatrix<T> const &ListVect,
                            MyVector<T> const &eVect, std::ostream &os) {
  int nbVect = ListVect.rows();
  ...
  std::optional<MyVector<T>> opt =
      SolutionMatNonnegative(ListVectNew, eVectNew, os);
  if (!opt) {
    return {};
  }
...
vectface Kernel_GetFullRankFacetSet(const MyMatrix<T> &EXT, std::ostream &os) {
  ...
  MyMatrix<T> EXTsel =
      SetIsobarycenter(EXTsel_pre2); // Essential step, selecting a subset
                                     // offsets the isobarycenter.
#ifdef TIMINGS_FULL_RANK_FACET_SET
...
bool is_full_dimensional_bounded_polytope(MyMatrix<T> const &FAC,
  ...
  IneqFund(0) = 1;
  std::optional<MyVector<T>> opt =
      SolutionMatStrictlyPositive(FAC, IneqFund, os);
  if (opt) {
    return true;
  } else {
    return false;
 ...
 */
template <typename T>
bool has_empty_intersection(MyMatrix<T> const &EXT1, MyMatrix<T> const &EXT2,
                            std::ostream &os) {
  int dim = EXT1.cols();
...
// Given a cone C whose extreme rays are the rows of EXT and an ambient
// direction eVect, find a facet inequality of C that is strictly violated
// by eVect (i.e. its scalar product with eVect is negative). Returns the
// (nbCol-dim) facet-normal vector, oriented outward per the convention
// of FindFacetInequality.
//
// Algorithm. The LP finds an extreme ray of the dual cone C*, i.e. a
...
//
// * Slow path: NSP.rows() > 1. The LP returned an interior point of a
//   higher-dimensional face F of C*. Restrict the non-incident rows of
//   EXT to F's coordinates (M = EXT[!A] * NSP^T), then use
//   FindOneInitialVertex on the restricted cone to descend to a vertex.
//   The extra tight rows picked up there, unioned with A, give a full-
//   rank incidence set from which FindFacetInequality recovers the facet
//   normal. FindOneInitialVertex is robust (its internal
