...
std::pair<bool, permlib::Permutation> PERMLIB_TestEquivalenceGeneralNaked(
    int const &n, PermutationGroupPtr const &group, Face const &eList1,
    Face const &eList2, int const &eMethod) {
  ...
  if (eMethod == 0)
    mappingElement = permlib::setImage_classic(
        *group, eSet1.begin(), eSet1.end(), eSet2.begin(), eSet2.end());
  if (eMethod == 1)
    mappingElement = permlib::setImage_partition(
        *group, eSet1.begin(), eSet1.end(), eSet2.begin(), eSet2.end());
  if (mappingElement) {
...
std::pair<bool, permlib::Permutation>
PERMLIB_TestEquivalenceGeneral(int const &n, PermutationGroupPtr const &group,
                               Tint const &grp_size, Face const &eList1,
                               Face const &eList2) {
  Tint MaxSize = 10000;
  int eMethod = 1;
  if (grp_size < MaxSize)
...
template <typename Tint>
bool PERMLIB_TestEquivalence(int const &n, PermutationGroupPtr const &group,
                             Tint const &grp_size, Face const &eList1,
                             Face const &eList2) {
  return PERMLIB_TestEquivalenceGeneral(n, group, grp_size, eList1, eList2)
      .first;
}

Face PERMLIB_Canonicalization(int const &n, PermutationGroupPtr const &group,
                              Face const &eList) {
...
PermutationGroupPtr
PERMLIB_GetStabilizer_general(PermutationGroupPtr const &group,
                              Face const &eList, int const &opt) {
...
template <typename Tint>
PermutationGroupPtr PERMLIB_GetStabilizer(PermutationGroupPtr const &group,
                                          Tint const &grp_size,
                                          Face const &eList) {
  Tint MaxSize = 10000;
  if (grp_size < MaxSize) {
...
template <typename Tint_inp> struct TheGroupFormat {
private:
  int n;
  Tint_inp e_size;
  PermutationGroupPtr group;
  TheGroupFormat(int const &_n, Tint_inp const &_size,
                 PermutationGroupPtr const &_group)
      : n(_n), e_size(_size), group(_group) {}

public:
  using Tint = Tint_inp;
  using Telt = permlib::Permutation;
  TheGroupFormat(std::vector<permlib::Permutation> const &ListPerm,
                 int const &n_inp)
      : n(n_inp) {
    ...
    for (auto &eGen : ListPerm) {
      ...
      generatorList.push_back(
          permlib::Permutation::ptr(new permlib::Permutation(v)));
    }
  ...
  std::pair<bool, permlib::Permutation>
  RepresentativeAction_OnSets(Face const &f1, Face const &f2) const {
...
namespace std {
template <> struct hash<permlib::Permutation> {
...
MyFormTransversal GetListPermutation(
    PermutationGroupPtr TheGRP,
    permlib::SchreierTreeTransversal<permlib::Permutation> const &eTrans) {
  permlib::dom_int n = TheGRP->n;
  permlib::dom_int eElt = eTrans.element();
  std::unordered_set<permlib::Permutation, std::hash<permlib::Permutation>>
      ListPermWork;
  ...
  std::function<void(permlib::dom_int, permlib::Permutation)> fInsert =
      [&](permlib::dom_int const &eVal,
          permlib::Permutation const &ePerm) -> void {
...
template <typename Tint> struct OrbitMinimumArr {
  int n;
  Tint GRPsize;
  std::vector<MyFormTransversal> ListTrans;
};
...
template <typename Tint> struct ResultMinimum {
  Face eMin;
  Tint OrbitSize;
};

template <typename Tint>
ResultMinimum<Tint> GetMinimumRecord(OrbitMinimumArr<Tint> const &ArrMin,
                                     Face const &eFace) {
...
bool TestBelongingInGroup(IteratorGrp const &eIter,
                          permlib::Permutation const &ePerm) {
  permlib::Permutation eWork = ePerm;
  int nbClass = eIter.ListTrans.size();
  std::function<bool(permlib::dom_int, int)> fUpdate =
      [&](permlib::dom_int const &eElt, int const &i) -> bool {
...
template <typename Tint>
bool IsSubgroup(TheGroupFormat<Tint> const &g1,
                TheGroupFormat<Tint> const &g2) {
