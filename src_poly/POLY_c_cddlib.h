...
template<typename T>
inline typename std::enable_if<is_mpq_class<T>::value,vectface>::type DualDescription_incd(MyMatrix<T> const&TheEXT)
{
  return DualDescription_incd_mpq_class(TheEXT);
}

template<typename T>
inline typename std::enable_if<is_boost_mpq_rational<T>::value,vectface>::type DualDescription_incd(MyMatrix<T> const&TheEXT)
{
  return DualDescription_incd_boost_mpq_rational(TheEXT);
}

template<typename T>
inline typename std::enable_if<(not is_mpq_class<T>::value) && (not is_boost_mpq_rational<T>::value),vectface>::type DualDescription_incd(MyMatrix<T> const&TheEXT)
{
