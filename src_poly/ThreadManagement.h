...
public:
  ...
  int MPU_NumberFree()
  {
    return MaxNumThread - numberRunningThread;
  }
  void incNRT(int const& TheId)
  {
    std::lock_guard<std::mutex> lk(mulNRT);
    numberRunningThread++;
    *(LogErr[TheId]) << "Call to incNRT: Now numberRunningThread=" << numberRunningThread << "\n";
    *(LogErr[TheId]) << "MaxNumThread=" << MaxNumThread << " numberSpannedThread=" << numberSpannedThread << "\n";
  }
  void decNRT(int const& TheId)
  {
    std::lock_guard<std::mutex> lk(mulNRT);
    numberRunningThread--;
    *(LogErr[TheId]) << "Call to decNRT: Now numberRunningThread=" << numberRunningThread << "\n";
    *(LogErr[TheId]) << "MaxNumThread=" << MaxNumThread << " numberSpannedThread=" << numberSpannedThread << "\n";
  }
  int MPU_GetId()
  {
