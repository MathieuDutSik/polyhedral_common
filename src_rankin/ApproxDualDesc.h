...
/*
  ...
  */
template<typename T>
vectface ReduceVectfaceApproximate(MyMatrix<T> const& EXT, vectface const& vf, T const& tolThr, T const& tolFacet) {
  int n_row = EXT.rows();
  int dim = EXT.cols();
  std::unordered_set<Face> set;
  for (auto & eFace : vf) {
    std::vector<size_t> eList = FaceToVector<size_t>(eFace);
    SelectionRowCol<T> eSelect = TMat_SelectRowColMaxPivot_subset(EXT, eList, tolThr);
    if (eSelect == dim - 1) {
      Face FullFace(n_row);
