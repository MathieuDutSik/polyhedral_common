...
/*
  ...
  we have
  HessPhi(A + dA, dB) = det(A + dA) Tr((A+dA)^{-1} dB)
 ...
 */
template<typename T, typename Tint>
MyMatrix<T> FindApproximateMatrix(MyMatrix<T> const& A0, std::vector<MyMatrix<T>> const& B_k, std::vector<MyMatrix<Tint>> const& ListP, tol const& maxDeltaDet) {
  ...
  for (int i_space=0; i_space<n_space; i_space++) {
    ...
    for (int i_basis=0; i_basis<dim_space; i_basis++) {
      MyMatrix<T> B_trans = ListP[i_space] * B_k[i_basis] * ListP[i_space].transpose();
    ...
    for (int i_space=0; i_space<n_space; i_space++) {
      ...
      for (int i_basis=0; i_basis<dim_space; i_basis++) {
        ...
        for (int j_basis=0; j_basis<=i_basis; j_basis++) {
          ...
          T h2 = 2 * det * (det - 1) * (prod1.trace() * prod2.trace() - prod12.trace());
          T h = h1 + h2;
