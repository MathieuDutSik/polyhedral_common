...
template <typename T> struct AttackScheme {
  int h;
  MyMatrix<T> mat;
  int sign;
};
...
AttackScheme<T> INDEF_FORM_GetAttackScheme(MyMatrix<T> const &Qmat,
                                           std::ostream &os) {
  DiagSymMatSymbolic DiagInfo = DiagonalizeSymmetricMatrixSymbolic(Qmat, os);
...
template <typename T> bool INDEF_FORM_IsEven(MyMatrix<T> const &Qmat) {
...
template <typename T> struct INDEF_InvariantQ {
  int n;
  T eDet;
  int nbPlus;
  int nbMinus;
...
template <typename T> struct INDEF_InvariantQV {
  int eRank;
  T eNorm;
  T index;
  T divisor;
...
size_t INDEF_FORM_Invariant_IsotropicKplane_Raw(MyMatrix<T> const &Qmat,
  ...
  for (int u = 0; u < k; u++) {
    ...
    if (opt) {
      ...
      std::cerr
          << "eV should belong to the space by the virtue of being isotropic\n";
...
template <typename T> MyMatrix<T> ExpandMatrix(MyMatrix<T> const &M) {
  int n = M.rows();
