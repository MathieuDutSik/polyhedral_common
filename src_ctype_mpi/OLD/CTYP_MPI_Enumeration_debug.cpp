...
int main() {
    ...
    for (int u = 0; u < MaxNumberFlyingMessage; u++) {
      ...
      if (stat) {
        ...
        if (stat->error() != 0) {
          ...
          std::cerr << "f_stat.count_hi_and_cancelled = " <<
          f_stat.count_hi_and_cancelled << "\n"; std::cerr << "f_stat.MPI_SOURCE
          = " << f_stat.MPI_SOURCE << "\n"; std::cerr << "f_stat.MPI_TAG = " <<
          f_stat.MPI_TAG << "\n"; std::cerr << "f_stat.MPI_ERROR = " <<
          f_stat.MPI_ERROR << "\n";
          */

          std::cerr << "stat->tag() = " << stat->tag() << "\n";
          std::cerr << "stat->source() = " << stat->source()
                    << " irank=" << irank << "\n";
  ...
  auto fInsert = [&](PairExch<Tint> const &ePair) -> void {
    ...
    log << "Inserting New ctype" << ePair.eCtype
        << " idxMatrixCurrent=" << idxMatrixCurrent << " Obtained from "
        << ePair.eIndex << "END" << std::endl;
    std::cerr << "Inserting new form, now we have |ListCasesNotDone|="
              << ListCasesNotDone.size()
              << " |ListCasesDone|=" << ListCasesDone.size() << "\n";
    //    std::cerr << "idxMatrixCurrent=" << idxMatrixCurrent << " eCtype = "
    //    << ePair.eCtype << "\n";
    idxMatrixCurrent++;
  };
  auto GetUndoneEntry =
      [&]() -> boost::optional<std::pair<TypeCtypeExch<Tint>, int>> {
    auto it1 = ListCasesNotDone.begin();
    if (it1 != ListCasesNotDone.end()) {
      std::pair<TypeCtypeExch<Tint>, int> ePair = {it1->first,
                                                   it1->second.idxMatrix - 1};
  ...
  auto ClearUnsentAsPossible = [&]() -> void {
    ...
    while (true) {
      ...
      std::cerr << "Ctype=" << ListMesg[idx].eCtype
                << " index=" << ListMesg[idx].eIndex << "\n";
      pos--;
    }
  };
  ...
  std::cerr << "Reading finished, we have |ListCasesDone|="
            << ListCasesDone.size()
            << " |ListCasesNotDone|=" << ListCasesNotDone.size() << "\n";
  //
  ...
  while (true) {
    ...
    if (prob) {
      ...
      if (stqtic_cast<int>(ListMatrixUnsent.size()) < MaxStoredUnsentMatrices) {
      ...
      if (DoSomething) {
        boost::optional<std::pair<TypeCtypeExch<Tint>, int>> eReq =
            GetUndoneEntry();
        if (eReq) {
          StatusNeighbors[irank] = 0;
          //          std::cerr << "eReq is non zero\n";
          SetMatrixAsDone(eReq->first);
          //          std::cerr << "irank=" << irank << " eCtype=" <<
          //          eReq->first << "\n";
          ...
          std::cerr << "Number of Adjacent for idxMatrixF=" << idxMatrixF
