...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    int posNature = PositionVect({std::string("text"), std::string("netcdf")}, NatureInput);
    if (posNature == -1) {
    ...
    bool ApplyCanonicalization = BlDATA.ListBoolValues.at("ApplyCanonicalization");
    //
    ...
    auto AppendSingleCtype=[&](MyMatrix<Tint> const& M, int const& NbAdj, size_t const& iProc) -> void {
      ...
      if (posType == 0)
        AppendSingleCtype_T<Tint,int8_t>(M, NbAdj, pos, ListVar_Ctype[iProc], ListVar_NbAdj[iProc]);
      if (posType == 1)
        AppendSingleCtype_T<Tint,int16_t>(M, NbAdj, pos, ListVar_Ctype[iProc], ListVar_NbAdj[iProc]);
      if (posType == 2)
        AppendSingleCtype_T<Tint,int32_t>(M, NbAdj, pos, ListVar_Ctype[iProc], ListVar_NbAdj[iProc]);
      if (posType == 3)
        AppendSingleCtype_T<Tint,int64_t>(M, NbAdj, pos, ListVar_Ctype[iProc], ListVar_NbAdj[iProc]);
      NbWrite[iProc] = pos + 1;
    };
    //
    ...
    if (posNature == 1) {
      for (size_t iProcI=0; iProcI<NprocInput; iProcI++) {
        ...
        if (nbDim_Ctype != 3 || nbDim_NbAdj != 1) {
          std::cerr << "Ctype should have dimension 3 and nb_adjacent dimension 1\n";
          throw TerminalException{1};
        }
