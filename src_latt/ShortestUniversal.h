...
resultCVP<T,Tint> CVPVallentinProgram_choice(MyMatrix<T> const& GramMat, MyVector<T> const& eV, std::string const& NameMeth)
{
  bool DoCheck=false;
  if (DoCheck) {
    ...
    if (res1 != res2) {
      ...
      std::cerr << "|res1.ListVect|=" << res1.ListVect.rows() << " |res2.ListVect|=" << res2.ListVect.rows() << "\n";
...
template<typename T, typename Tint>
resultCVP<T,Tint> CVPVallentinProgram(MyMatrix<T> const& GramMat, MyVector<T> const& eV)
{
  return CVPVallentinProgram_exact<T,Tint>(GramMat, eV);
}
