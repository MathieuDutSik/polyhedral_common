...
std::pair<std::vector<Tint>, std::vector<Tint>> lll(std::vector<Tint> G,
                                                    int n) {
  Tfloat delta = 0.99;
  Tfloat eta = 0.501;
  int kappa = 1;
  ...
  while (kappa < n) {
    ...
    for (int i = kappa - 1; i >= 0; i--) {
      if (std::fabs(mu[ind(kappa, i, n)]) > eta) {
        ...
        // update G
        G[ind(kappa, kappa, n)] -=
            2 * rnd * G[ind(kappa, i, n)] - rnd * rnd * G[ind(i, i, n)];
        for (int j = 0; j < kappa; j++) {
    ...
    // lovasz condition
    if ((delta - mu[ind(kappa, kappa - 1, n)] * mu[ind(kappa, kappa - 1, n)]) *
            r[kappa - 1] <=
        r[kappa]) {
      kappa++;
    } else {
