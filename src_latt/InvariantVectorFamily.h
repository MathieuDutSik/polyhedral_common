...
MyMatrix<Tint> CharVectSet_MinimalVectors(MyMatrix<T> const &GramMat,
  ...
  return matrix_duplication(RecSHV.SHV);
}
...
MyMatrix<Tint> CharacteristicVectorSetCV(MyMatrix<T> const &GramMat,
  ...
  if (r == n) {
    ...
    return CharacteristicVectorSetWellRoundedCV<T, Tint>(GramMat, os);
  }
  CharVectSet_Accumulator<Tint> acc;
  ...
  MyMatrix<Tint> Vwr = CharacteristicVectorSetWellRoundedCV<T, Tint>(Gram1, os);
  ...
  // correctness.
  MyMatrix<Tint> V2 = CharacteristicVectorSetCV<T, Tint>(Gram2, strict_paper, os);
  CVPSolver<T, Tint> solver(GramMat, os);
