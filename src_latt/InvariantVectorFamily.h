...
// group generated by ListGen (acting by v -> g v), ordered by increasing
// GramMat-norm v^T GramMat v. Vectors are enumerated shell by shell with the
// norm increasing by GetSmallestIncrement(GramMat), so there is no bound
// parameter. (Antipodes -v and v fall in the same orbit exactly when -Id is in
// the group, which holds e.g. for an arithmetic automorphism group.)
template <typename T, typename Tint>
