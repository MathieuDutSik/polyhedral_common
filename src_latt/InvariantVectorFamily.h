...
/*
  The norms v^T eMat v of the integral vectors span the Z module generated by
  the diagonal entries and twice the off diagonal ones. Over a rational scalar
  type that module is cyclic and this returns its generator, so every
  achievable norm is one of its multiples and stepping by it enumerates the
  norms exactly, missing none.

  That is a property of the rationals and not of the basis: a change of basis
  replaces the generators by Z combinations of them and back, so the module,
  and with it the value returned here, is an invariant of the lattice. The
  shell enumeration relies on both halves of that.

  Over an algebraic scalar type the module has rank the degree of the field
  and is dense in the reals, so it has no smallest positive element to return
  and no gcd exists. GetShellStep below is what the shell enumeration uses,
  and it takes a different route there.
 */
template <typename T> T GetSmallestIncrement(MyMatrix<T> const &eMat) {
  static_assert(is_implementation_of_Z<T>::value ||
                    is_implementation_of_Q<T>::value,
                "The gcd of the Gram entries generates the module of the "
                "norms only over a rational scalar type");
  int n = eMat.rows();
  T eGcd = eMat(0, 0);
...
/*
  The step by which the shell enumeration advances its norm.

  Over a rational scalar type it is the generator of the module of the norms,
  so the enumeration can step through the norms exactly.

  Over an algebraic scalar type no such generator exists and the step becomes
  a step of the radius, ShellFamilyBuilder taking whole balls rather than
  exact shells. What is needed of it is then only that it be positive, that it
  not overshoot, and above all that it be an invariant of the lattice: the
  family the builder returns is the input of a canonical form, so two bases of
  the same lattice have to produce the same radii.

  The minimum of the Gram entries is not such an invariant. It is a property
  of the basis, and on a skewed basis it comes out far above the lattice
  minimum -- above GetMaxNorm even, which makes the builder give up before
  enumerating anything. The lattice minimum is used instead: it is an
  invariant by definition, it is positive, and the first ball it gives is
  non-empty by construction since the shortest vectors are in it.
 */
template <typename T, typename Tint>
T GetShellStep(MyMatrix<T> const &eMat, CVPSolver<T, Tint> const &solver) {
  if constexpr (is_implementation_of_Z<T>::value ||
                is_implementation_of_Q<T>::value) {
    return GetSmallestIncrement(eMat);
  } else {
    return solver.shortest_vectors().min;
  }
}
...
struct ShellFamilyBuilder {
  ShellFamilyBuilder(MyMatrix<T> const &GramMat, Ffinal f_final,
      ...
      : f_final_(f_final), f_invariant_(f_invariant), n_(GramMat.rows()),
        solver_(GramMat, os),
        incr_(GetShellStep<T, Tint>(GramMat, solver_)),
        max_norm_(GetMaxNorm<T, Tint>(GramMat, os)), norm_(incr_),
        SHVret_(0, GramMat.rows()),
        fi_ret_(TrivFundamentalInvariant<Tint>()), done_(false),
        exhausted_(false) {}
  ...
  void one_shell(std::ostream &os) {
    ...
    MyMatrix<Tint> SHVtest = next_family(radius);
...
private:
  /*
    The family to test at the current value of norm_.

    Over a rational scalar type the step is a gcd of the Gram entries, so
    every achievable norm is one of its multiples and the shell of norm
    exactly norm_ is the only thing not already collected: the family grows by
    appending that shell to what is there.

    Over an algebraic scalar type the step is only a step of the radius, and
    the norms strictly between two of its multiples are not empty, so the
    shell of exactly norm_ would miss them. The ball of radius norm_ is taken
    whole instead. It contains the previous one, so it replaces rather than
    extends -- the two enumerations agree on their conventions, both returning
    the nonzero vectors one per antipodal pair.
   */
  MyMatrix<Tint> next_family(T const &radius) const {
    if constexpr (rational_scalars) {
      std::vector<MyVector<Tint>> ListVect =
          solver_.fixed_norm_vectors(radius);
      ...
      std::vector<MyVector<Tint>> ListVect =
          solver_.at_most_norm_vectors(radius);
      return MatrixFromVectorFamilyDim(n_, ListVect);
    }
  }

  // Whether the norms of the integral vectors span a cyclic Z module, which
  // is what lets the enumeration step through them exactly.
  static constexpr bool rational_scalars =
      is_implementation_of_Z<T>::value || is_implementation_of_Q<T>::value;

  Ffinal f_final_;
  Finvariant f_invariant_;
  int n_;
  // Before incr_, which is initialized from it: over an algebraic scalar type
  // the step is the lattice minimum, which the solver computes.
  CVPSolver<T, Tint> solver_;
  T incr_, max_norm_, norm_;
  MyMatrix<Tint> SHVret_;
  FundInvariantVectorFamily<Tint> fi_ret_;
  bool done_;
  // Set once the ball of radius max_norm_ has been taken, so that a further
  // call reports the failure rather than enumerating it again.
  bool exhausted_;
};
