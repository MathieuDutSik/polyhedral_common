...
template<typename T>
CP<T> CenterRadiusDelaunayPolytopeGeneral(MyMatrix<T> const& GramMat, MyMatrix<T> const& EXT)
{
  static_assert(is_ring_field<T>::value, "Requires T to be a field");
  int n=GramMat.rows();
  /*  std::cerr << "GramMat=\n";
  ...
  for (int iVert=1; iVert<nbVert; iVert++) {
    ...
    T Sum=EvaluationQuadForm<T,T>(GramMat, eV) - EvaluationQuadForm<T,T>(GramMat, fV);
    ListB(idx)=Sum;
    for (int i=0; i<n; i++) {
      T vSum=0;
      for (int j=0; j<n; j++)
