...
static void red(long dim, double **reduced_basis, double **mu, long k, long l) {
  double q;
  long i;
...
static void swap(long dim, double **reduced_basis, double **o_basis,
                 double **mu, double *B, double *b0, long k, long k_max) {
  long i, j;
  double mu0, B0, t;
...
void computeLLLReducedBasis(long dim, double **basis, double **reduced_basis,
                            int check) {
  long i, j, k, k_max, l;
  double prod, prod1, prod2, *b0, *B, **mu, **o_basis;
  ...
  if (check) {
    ...
    for (i = 1; i < dim; i++) {
      ...
      if (prod1 < (LOVASZ_CONSTANT - mu[i][i - 1] * mu[i][i - 1]) * prod2) {
