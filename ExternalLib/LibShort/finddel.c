...
void solve_lp(int dim, int *objective_function, int *inequalities,
              int inequalities_count, double *vertex) {
  int row, col;
  long num[dim + 1], den[dim + 1];
  double det;
  lrs_dic *P;
  lrs_dat *Q;
...
void find_random_delone_polytope(int dim, shvec_request request,
                                 shvec_info info, double **gram,
                                 int print_radii) {
  int i, j, k, is_finished, finished, *rand_direction, inequalities_count,
      *inequalities;
  double *vertex1, *vertex2, **inv_gram, norm, radius;
  ...
  for (i = 0; i < dim; i++) {
    ...
    for (j = 1; j < dim + 1; j++)
      if (i == j - 1)
        inequalities[(dim + 1) * i + j] = 1;
      else
        inequalities[(dim + 1) * i + j] = 0;
  }
  ...
  for (i = 0; i < dim; i++) {
    ...
    for (j = 1; j < dim + 1; j++)
      if (i == j - 1)
        inequalities[(dim + 1) * (dim + i) + j] = -1;
      else
        inequalities[(dim + 1) * (dim + i) + j] = 0;
  }
  ...
  do {
    ...
    for (k = 0; k < info->short_vectors_number; k++) {
      inequalities = (int *)realloc(
          inequalities, (dim + 1) * (inequalities_count + 2) * sizeof(int));
      if (inequalities == NULL)
        die_finddel("inequalities == NULL.\n");
...
int main(int argc, char **argv) {
  ...
  double **gram_matrix;
  int tries, seed, dim, print_radii, c, i, j, t;
