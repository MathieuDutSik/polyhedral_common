...

{
  ...
  ***************************************************/
  ...
  for (index = lastdv + 1; index <= m + d; index++) {
    ...
    if (debug)
      fprintf(lrs_ofp, "\ncheck index=%ld, inequality=%ld, redineq=%ld", index,
              ineq, redineq[ineq]);
    if (redineq[ineq] == ONE)
...
long lrs_init(char *name) /* returns TRUE if successful, else FALSE */
{
  /*
  printf ("%s", name);
  printf (TITLE);
...
#ifdef MP
  fprintf (lrs_ofp, " max digits=%ld/%ld", DIG2DEC (lrs_record_digits), DIG2DEC
(lrs_digits)); #endif
...
/* no_output is TRUE supresses output headers   */
{
  ...
  if (!no_output || Q->debug) {
    /*
    if (Q->voronoi)
      fprintf (lrs_ofp, "\n*Voronoi Diagram: Voronoi vertices and rays are
    output"); if (hull) fprintf (lrs_ofp, "\nH-representation"); else fprintf
    (lrs_ofp, "\nV-representation");
    */
  ...
  if (Q->maximize || Q->minimize) {
    ...
    if (Q->lponly) {
      /*
     fprintf (lrs_ofp, "\nNumber of pivots for optimum solution:
     %ld",Q->count[3]);
      */
      if (Q->verbose) {
        printA(D, Q);
...

{
  ...
  while ((j < d) || (D->B[m] != m)) /*main while loop for getnextbasis */
  {
    ...
    if (Q->truncate &&
        negative(D->A[0][0])) /* truncate when moving from opt. vertex */
      backtrack = TRUE;
...
/* col is output column being printed */
{
  ...
  if (hull)
    fprintf(lrs_ofp, "\nF#%ld B#%ld h=%ld vertices/rays ", count[0], count[2],
            P->depth);
  else if (Q->voronoi)
    fprintf(lrs_ofp, "\nV#%ld R#%ld B#%ld h=%ld data points ", count[1],
            count[0], count[2], P->depth);
  else
    fprintf(lrs_ofp, "\nV#%ld R#%ld B#%ld h=%ld facets ", count[1], count[0],
            count[2], P->depth);
...
void lrs_printtotals(lrs_dic *P, lrs_dat *Q) {
  ...
  if (hull) /* output things that are specific to hull computation */
  {
        ...
        homogeneous) /* don't count column 1 as redundant if homogeneous */
    {
      ...
      fprintf(lrs_ofp, " facets+linearities=%ld",
              nredundcol - homogeneous + count[0]);
    }

    if ((cest[2] > 0) || (cest[0] > 0)) {
      fprintf(lrs_ofp, "\n*Estimates: facets=%g bases=%g", count[0] + cest[0],
              count[2] + cest[2]);
      if (Q->getvolume) {
    ...
    if (Q->restart || Q->allbases ||
        (count[0] > 1 && !Q->homogeneous && !Q->polytope))
  ...
  } else /* output things specific to vertex/ray computation */
  {
    ...
    if ((cest[2] > 0) || (cest[0] > 0)) {
      fprintf(lrs_ofp, "\n*Estimates: vertices=%g rays=%g bases=%g",
              count[1] + cest[1], count[0] + cest[0], count[2] + cest[2]);
...
/*  Estimation function */
/************************/
void lrs_estimate(lrs_dic *P, lrs_dat *Q)
  ...
  for (runcount = 1; runcount <= Q->runs;
    ...
    while (nchild != 0) /* while not finished yet */
    {
      ...
      if (nchild > 0) /*reverse pivot found choose random child */
      {
        ...
        rays = 0;
        for (col = 1; col <= d; col++)
          if (negative(A[0][col]) && (ratio(P, Q, col) == 0) &&
              lexmin(P, Q, col))
            rays++;
 ...
 */
void save_basis(lrs_dic *P, lrs_dat *Q) {
  int i;
  /* assign local variables to structures */
  long *C = P->C;
  long d;
...
void print_basis(FILE *fp, lrs_dat *global) {
  ...
  if (global->saved_flag) {
    ...
    fprintf(fp, "V#%ld R#%ld B#%ld h=%ld facets ", global->saved_count[1],
