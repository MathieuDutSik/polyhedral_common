...
int insertBound(shvec_info info, int *vector, int *c, int coset, double norm) {
  ...
  if (number % SHVEC_MAX == 0) {
    ...
    info->short_vectors[number] = SHVEC_malloc(dim * sizeof(int) * SHVEC_MAX);
    for (i = 1; i < SHVEC_MAX; i++) {
      info->short_vectors[i + number] = info->short_vectors[number] + i * dim;
    }
  ...
  if (info->request.ints.integral && info->request.ints.ComputeTheta &&
      !coset) {
    ...
    if (shell_bound % SHVEC_MAX == 0) {
      info->shells[vector_norm] = SHVEC_realloc(
  ...
  intMultiplySquareMatrixVector(dim, info->request.ints.transformation,
...
int insertThetaSeries(shvec_info info, int *vector, int *c, int coset,
                      double norm) {
  int vector_norm, i;
  info->short_vectors_count++;
  vector_norm = ROUND(norm);
...
int insertStop(shvec_info info, int *vector, int *c, int coset, double norm) {
  info->minimum = norm;
  return STOP_COMPUTATION;
}
...
int computeIt(shvec_info info, int (*insert)()) {
  ...
  while (not_finished) {
    ...
    if (X[i] <= L[i]) {
      ...
      if (i == 0) {
        ...
        result =
            insert(info, x, c, coset,
                   bound - T[0] +
                       q[0][0] * (X[0] + C[0] + U[0]) * (X[0] + C[0] + U[0]));
        if (result == STOP_COMPUTATION)
          goto urgent_stop;
      } else {
        i--;
        sum = 0.0;
        for (j = i + 1; j < dim; j++)
          sum += q[i][j] * (X[j] + C[j]);
        U[i] = sum;
        T[i] = T[i + 1] - q[i + 1][i + 1] * (X[i + 1] + C[i + 1] + U[i + 1]) *
                              (X[i + 1] + C[i + 1] + U[i + 1]);
        needs_new_bound = 1;
      }
    } else {
...
static int computeMinimum(shvec_info info) {
  ...
  if (coset)
    doubleVectorNorm(dim, info->request.ints.gram_matrix, C, &(info->minimum));
  else {
  ...
  if (coset || !info->request.ints.integral)
    step_size = SHVEC_STEP_EPSILON;
  else if (info->request.ints.even)
    step_size = 2.0;
  else
    step_size = 1.0;
...
static void copyShvecReq(shvec_request req_from, shvec_request req_to) {
  int dim;
...
void initShvecReq(int dim, double **gram_matrix,
                  int (*cholesky_decomposition)(), int check,
                  shvec_request request) {
  double **up_trig_mat, **inv_up_trig_mat, **double_transformation,
      **reduced_mat, **inv_reduced_mat, **q, *row_norm, temp;
  int success, i, j, t;
  ...
  success = request->ints.cholesky_decomposition(dim, gram_matrix, up_trig_mat);
  if (!success)
...
void computeShvec(shvec_request request, shvec_info info) {
  ...
  case SHVEC_MODE_SHORTEST_VECTORS:
    if (request->bound != 0.0 || request->number < 0)
      die_shvec(
          "shvec.c (computeShvec): wrong options MODE_SHORTEST_VECTORS!\n");
    break;
