...
static void pivotSearch(int dim, double **mat, int row, int *pivot_row) {
  int i;
  double pivot;
...
static void solveRecursively(int dim, double **u_mat, double *b, double *x) {
  int i, j;
...
void doubleMakeSquareMatrix(int dim, double ***matrix) {
  int i;
...
void doubleDestroySquareMatrix(int dim, double ***matrix) {
  laFree((*matrix)[dim]);
  laFree(*matrix);
  *matrix = NULL;
}

void doubleCopySquareMatrix(int dim, double **from, double **to) {
  int i;
...
void doubleAddSquareMatrix(int dim, double factor, double **from, double **to) {
  int i, j;
...
void doubleSetIdentitySquareMatrix(int dim, double **matrix) {
  int i, j;
...
void doubleMultiplySquareMatrixVector(int dim, double **matrix, double *vec1,
                                      double *vec2) {
  int i, j;
...
void doubleRowAddSquareMatrix(int dim, double **matrix, double factor, int from,
                              int to) {
  int i;
...
void doubleRowChangeSquareMatrix(int dim, double **matrix, int index1,
                                 int index2) {
  int i;
  double t;
...
int doubleComputeLUSquareMatrix(int dim, double **mat, double **l_mat,
                                double **u_mat, double **p_mat) {
  int i, j, k, pivot_row;
  double temp;
  ...
  for (i = 0; i < dim; i++) {
    ...
    for (j = i + 1; j < dim; j++) {
      for (k = i + 1; k < dim; k++)
        u_mat[j][k] = u_mat[j][k] - u_mat[j][i] / u_mat[i][i] * u_mat[i][k];
      u_mat[j][i] = 0.0;
    }
  } /* for i */
...
void doubleTransposeSquareMatrix(int dim, double **mat, double **t_mat) {
  int i, j;
  double temp;
...
void doubleSolveLUSquareMatrix(int dim, double **l_mat, double **u_mat,
                               double **p_mat, double *b, double *x) {
  double *c, *d;
  double **inv_l_mat;
...
int doubleInverseSquareMatrix(int dim, double **mat, double **inv_mat) {
  int i, col, ok;
  double **l_mat, **u_mat, **p_mat;
  double *x_vec, *e_vec;
...
void doubleInverseLowerSquareMatrix(int dim, double **l_mat,
                                    double **inv_l_mat) {
  double *e_vec;
  int i, row, col;
  ...
  for (col = 0; col < dim; col++) {
    ...
    for (row = 0; row < dim; row++) {
      ...
      for (i = 0; i < row; i++)
        inv_l_mat[row][col] =
            inv_l_mat[row][col] - l_mat[row][i] * inv_l_mat[i][col];
...
void doubleInverseUpperSquareMatrix(int dim, double **u_mat,
                                    double **inv_u_mat) {
...
void doubleAbsoluteDeterminantSquareMatrix(int dim, double **l_mat,
                                           double **u_mat, double *det) {
  int i;
  double d_det;
...
int doubleComputeCholeskySquareMatrix(int dim, double **mat, double **u_mat) {
  int i, j, k, l;
...
void doubleComputeSymmetrizationSquareMatrix(int dim, double **mat,
                                             double **sym_mat) {
  int i, j, k;
...
void doubleMultiplySquareMatrix(int dim, double **a, double **b, double **c) {
  int i, j, k;
  double *t;
...
void doubleComputeRowNormSquareMatrix(int dim, double **mat, int row,
                                      double *row_norm) {
  int i;
...
void doublePermuteColumnsSquareMatrix(int dim, double **mat, int index1,
                                      int index2) {
  int i;
  double t;
...
void doublePermuteRowsSquareMatrix(int dim, double **mat, int index1,
                                   int index2) {
  int i;
  double t;
...
void double2intSquareMatrix(int dim, double **d_mat, int **l_mat) {
  int i, j;
...
void doublePrintSquareMatrix(int dim, double **mat) {
  int i, j;
...
void doublePrintGramMatrix(int dim, double **gram_matrix) {
  int i, j;
...
void doubleMakeVector(int dim, double **vector) {
  *vector = (double *)laMalloc(dim * sizeof(double));
}

void doubleDestroyVector(int dim, double **vector) {
  laFree(*vector);
  *vector = NULL;
}

void doubleCopyVector(int dim, double *from, double *to) {
  memcpy(to, from, dim * sizeof(double));
}

void doubleMultiplyVector(int dim, double factor, double *from, double *to) {
  int i;
...
void doubleAddVector(int dim, double factor, double *from, double *to) {
  int i;
...
void doubleInnerProduct(int dim, double *vector1, double *vector2,
                        double *product) {
  int i;
...
void doubleVectorNorm(int dim, double **gram_matrix, double *vector,
                      double *norm) {
  int i, j;
  double temp;
...
void doublePrintVector(int dim, double *vector) {
  int i;
...
int doubleIsMultipleVector(int dim, double *vector1, double *vector2) {
  int i, j, k;
...
void intMakeVector(int dim, int **vector) {
  *vector = (int *)laMalloc(dim * sizeof(int));
}

void intDestroyVector(int dim, int **vector) {
  laFree(*vector);
  *vector = NULL;
}

void intCopyVector(int dim, int *from, int *to) {
  memcpy(to, from, dim * sizeof(int));
}

void intPrintVector(int dim, int *vector) {
  int i;
...
void intInnerProductVector(int dim, int *vector1, int *vector2, int *product) {
  int i;
...
void intVectorNorm(int dim, double **gram_matrix, int *vector, double *norm) {
  int i, j;
  double temp;
...
void intMakeSquareMatrix(int dim, int ***matrix) {
  int i;
...
void intDestroySquareMatrix(int dim, int ***matrix) {
  laFree((*matrix)[0]);
  laFree(*matrix);
  *matrix = NULL;
}

void intCopySquareMatrix(int dim, int **matrix1, int **matrix2) {
  int i;
...
void intMultiplySquareMatrixVector(int dim, int **matrix, int *vec1,
                                   int *vec2) {
  int i, j;
...
void intPrintGramMatrix(int dim, int **gram_matrix) {
  int i, j;
...
void intPrintSquareMatrix(int dim, int **matrix) {
  int i, j;
