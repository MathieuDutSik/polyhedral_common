...
int main(int argc, char *argv[])
{
  try {
    ...
    if (argc != 7) {
      ...
      std::cerr << "SHORT_EnumerateCyclicCases n d iProc Nproc TheMethod [outfile]\n";
      std::cerr << "\n";
    ...
    //
    std::cerr << "n=" << n << " d=" << d << "\n";
    std::vector<std::vector<int>> ListCases=SHORT_GetCandidateCyclic_Optimized(n, d);
    ...
    for (int iCase=0; iCase<nbCase; iCase++) {
      int res=iCase % NProc;
      std::cerr << "iCase=" << iCase << " / " << nbCase << " NProc=" << NProc << " iProc=" << iProc << " res=" << res << "\n";
      if (res == iProc) {
